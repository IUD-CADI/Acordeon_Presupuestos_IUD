/* IUD · URLs fijas + sumas: Inicial, Definitiva, Recaudo, Acumulado */
...
function applyAndRender(){
  ...
  const dec = Math.max(0, Math.min(4, Number($('#decimales').value || 0)));
  const symbol = $('#moneda').value || '$';
  const totals = computeTotals(tree);
  renderGlobalTotals(totals, symbol, dec);

  // Acordeón por niveles
  renderAccordion(tree, symbol, dec);
}

document.addEventListener('DOMContentLoaded', ()=>{
  $('#btnReload').addEventListener('click', loadAll);
  $('#btnApply').addEventListener('click', applyAndRender);
  $('#btnDiag').addEventListener('click', ()=> $('#diag').classList.toggle('hidden'));
  loadAll();
});
