/* IUD · filtro por Mes + Totales globales (solo MASTER_URL y DATA_URL) */
...
const normKey = s => String(s||'')
  ...
  .replace(/[\u00A0\s]+/g,' ') // colapsa espacios y NBSP
  .trim().toLowerCase();
...
function diagnostics(headersM, headersD, mCols, dCols, match, totalM, totalD){
  $('#diag').innerHTML = `
    ...
    <p>Mapeo Ejecución → codigo=<code>${dCols.codigo||'-'}</code>, año=<code>${dCols.anio||'-'}</code>, mes=<code>${dCols.mes||'-'}</code>, ini=<code>${dCols.ini||'-'}</code>, recaudo=<code>${dCols.recaudo||'-'}</code>, acum=<code>${dCols.acum||'-'}</code></p>
...
function buildTree(masterRows, dataRows, mCols, dCols){
  ...
  const tree = new Map(); // nivel1 -> Map(nivel2 -> {ini, rec, acm})
  for(const r of dataRows){
    ...
    if(!sub.has(nivel2)) sub.set(nivel2, {ini:0, rec:0, acm:0});

    const node = sub.get(nivel2);
    node.ini += ini; node.rec += rec; node.acm += acm;
  }
  return tree;
}

function computeTotals(tree){
  let tIni=0, tRec=0, tAcm=0;
  for(const [, lvl2] of tree.entries()){
    for(const [, v] of lvl2.entries()){
      tIni += v.ini; tRec += v.rec; tAcm += v.acm;
    }
  }
  return { tIni, tRec, tAcm };
}

function renderGlobalTotals(totals, symbol, dec){
  const g = $('#global');
  const { tIni, tRec, tAcm } = totals;
  g.innerHTML = `
    <div class="global">
      <table>
        <thead>
...
async function loadAll(){
  ...
  try{
    ...
    const d = await fetchXlsx(DATA_URL);
